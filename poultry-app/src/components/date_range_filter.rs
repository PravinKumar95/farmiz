...
pub fn DateRangeFilter(props: DateRangeFilterProps) -> Element {
    ...
    rsx! {
        div { class: "flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-2.5 w-full py-1",
            // ── Preset Pills ──
            div { class: "flex items-center gap-1.5 overflow-x-auto flex-nowrap py-0.5 max-w-full touch-pan-x",
                ...
                for (key, label) in presets {
                    {
                        ...
                        rsx! {
                            button {
                                ...
                                r#type: "button",
                                class: if is_active {
                                    "px-3 py-1.5 rounded-lg bg-blue-600 dark:bg-blue-600 text-white text-xs font-semibold shadow-sm whitespace-nowrap shrink-0 transition-all cursor-pointer ring-1 ring-blue-500/50"
                                } else {
                                    "px-3 py-1.5 rounded-lg bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-200 hover:bg-stone-200/80 dark:hover:bg-stone-700/80 text-xs font-medium whitespace-nowrap shrink-0 transition-all cursor-pointer border border-stone-200/70 dark:border-stone-700/60"
                                },
                                onclick: {
                                    let k = k_str.clone();
                                    move |_| {
            ...
            div { class: "flex items-center gap-2 self-start lg:self-auto shrink-0 bg-stone-50 dark:bg-stone-800/60 p-1.5 rounded-lg border border-stone-200/70 dark:border-stone-700/60 text-xs",
                div { class: "flex items-center gap-1.5",
                    span { class: "text-stone-500 dark:text-stone-400 font-medium text-[11px]", "From" }
                    input {
                        r#type: "date",
                        class: "h-7 px-2 rounded border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-900 text-stone-800 dark:text-stone-200 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500",
                        value: "{current_start}",
                        onchange: {
                ...
                if has_filter {
                    button {
                        r#type: "button",
                        class: "px-2 py-1 rounded bg-stone-200 dark:bg-stone-700 hover:bg-stone-300 dark:hover:bg-stone-600 text-stone-700 dark:text-stone-300 font-medium text-[11px] transition-colors cursor-pointer ml-1",
                        onclick: move |_| {
