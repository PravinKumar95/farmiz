...
pub async fn sync_party_balance(pool: &PgPool, party_name: &str, party_id: Option<Uuid>, user_id: &str) {
    ...
    let egg_charges_res: Option<f64> = if let Some(pid) = party_id {
        sqlx::query_scalar("SELECT SUM(total_amount - received_amount) FROM egg_sales WHERE party_id = $1 AND user_id = $2")
            .bind(pid).bind(user_id).fetch_one(pool).await.unwrap_or(None)
    } else {
    ...
    let broken_charges_res: Option<f64> = if let Some(pid) = party_id {
        sqlx::query_scalar("SELECT SUM(amount - payment_received) FROM broken_egg_sales WHERE party_id = $1 AND user_id = $2")
            .bind(pid).bind(user_id).fetch_one(pool).await.unwrap_or(None)
    } else {
    ...
    let purchase_balance_res: Option<f64> = if let Some(pid) = party_id {
        sqlx::query_scalar("SELECT SUM(total_amount - advance_paid) FROM material_purchases WHERE party_id = $1 AND user_id = $2")
            .bind(pid).bind(user_id).fetch_one(pool).await.unwrap_or(None)
    } else {
        sqlx::query_scalar("SELECT SUM(total_amount - advance_paid) FROM material_purchases WHERE party_name = $1 AND user_id = $2")
            .bind(party_name).bind(user_id).fetch_one(pool).await.unwrap_or(None)
    };
...
pub async fn sync_employee_balance(pool: &PgPool, employee_name: &str, employee_id: Option<Uuid>, user_id: &str) {
    let advances_res: Option<f64> = if let Some(eid) = employee_id {
        sqlx::query_scalar("SELECT SUM(advance_given) FROM labor_records WHERE employee_id = $1 AND user_id = $2")
            .bind(eid).bind(user_id).fetch_one(pool).await.unwrap_or(None)
    } else {
