...
pub async fn get_dashboard_stats(
    ...
    let standard_sales: Option<f64> = sqlx::query_scalar(
        "SELECT SUM(total_amount) FROM egg_sales WHERE date = $1 AND (user_id = $2 OR user_id IS NULL)",
    )
    .bind(&today)
    .bind(&user.user_id)
    .fetch_one(&pool)
    ...
    let broken_sales: Option<f64> = sqlx::query_scalar(
        "SELECT SUM(amount) FROM broken_egg_sales WHERE date = $1 AND (user_id = $2 OR user_id IS NULL)",
    )
    .bind(&today)
    .bind(&user.user_id)
    .fetch_one(&pool)
    ...
    let eggs_sold: Option<i64> = sqlx::query_scalar(
        "SELECT SUM(total_eggs) FROM egg_sales WHERE date = $1 AND (user_id = $2 OR user_id IS NULL)",
    )
    .bind(&today)
    .bind(&user.user_id)
    .fetch_one(&pool)
    ...
    let purchases: Option<f64> = sqlx::query_scalar(
        "SELECT SUM(total_amount) FROM material_purchases WHERE date = $1 AND (user_id = $2 OR user_id IS NULL)",
    )
    .bind(&today)
    .bind(&user.user_id)
    .fetch_one(&pool)
    ...
    let active_parties: Option<i64> = sqlx::query_scalar(
        "SELECT COUNT(*) FROM parties WHERE user_id = $1 OR user_id IS NULL",
    )
    .bind(&user.user_id)
    .fetch_one(&pool)
    .await
